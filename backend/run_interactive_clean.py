"""
Clean Interactive Race Simulator
- 6 total decision points
- 4 tactical decisions (3 options each)
- 2 pit stop decisions (type lap number)
"""
...
def display_pit_window(window, current_lap):
    ...
    # Show visual timeline (next 12 laps)
    print(f"\n📅 PIT WINDOW TIMELINE:")
    timeline_str = ""
...
def run_clean_race():
    """Run race with 6 decision points"""

    print("="*70)
    print("🏎️  F1 INTERACTIVE RACE SIMULATOR")
    print("="*70)
    print("\n📋 GOAL: Beat Verstappen's Bahrain 2024 race time")
    print("📋 Make 6 strategic decisions throughout the race")
    print("📋 4 tactical decisions (3 options)")
    ...
    # Track planned pits
    planned_pits = []  # [(lap, compound), ...]
    pit_windows_offered = 0  # Track how many pit windows we've shown
    ...
    # Update tire model with new driving style
    from driving_style import DrivingStyleManager
    ...
    while current_lap <= sim.total_laps:
        # Execute planned pit stop
        if any(lap == current_lap for lap, _ in planned_pits):
            compound = next(comp for lap, comp in planned_pits if lap == current_lap)
            print(f"\n⏩ LAP {current_lap} - PITTING FOR {compound} TIRES")

            sim.state.tire_compound = compound
            sim.state.tire_age = 0  # Reset tire age!
        ...
        sim.state.tire_age += 1  # Increment tire age after simulating lap

        # Check for decision points
        max_laps = sim.tire_model.COMPOUND_WEAR_RATES[sim.state.tire_compound]['max_laps']
        tire_pct = sim.state.tire_age / max_laps
        has_upcoming_pit = any(current_lap < lap <= current_lap + 5 for lap, _ in planned_pits)
        ...
        # ========== DECISION 3: FIRST PIT WINDOW (~70% tire wear) ==========
        if tire_pct >= 0.70 and pit_windows_offered == 0 and not has_upcoming_pit:
            ...
            planned_pits.append((pit_lap, window['optimal_compound']))
            print(f"\n✅ Planned: Pit lap {pit_lap} for {window['optimal_compound']} tires")

            pit_windows_offered += 1
        ...
        # ========== DECISION 5: SECOND PIT WINDOW (~70% tire wear on stint 2) ==========
        if tire_pct >= 0.70 and pit_windows_offered == 1 and not has_upcoming_pit:
            laps_remaining = sim.total_laps - current_lap

            # Check if we have enough laps to make a second pit worth it
        ...
        # Progress indicator
        if current_lap % 10 == 0 and current_lap not in [1, 10, 30, 55]:
